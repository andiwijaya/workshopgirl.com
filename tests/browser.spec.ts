...
function wav(frequencies = [440], amplitude = 0.3, seconds = 1, stereo = false): Buffer {
...
async function upload(page: Page, buffer = wav(), name = 'tone.wav') {
...
async function expectTone(page: Page, hz = 440) {
...
async function syntheticMicrophone(page: Page, delayed = false) {
  await page.addInitScript(({ delayed }) => {
    ...
    Object.defineProperty(navigator.mediaDevices, 'getUserMedia', { configurable: true, value: async () => {
      ...
      oscillator.frequency.value = 440; gain.gain.value = 0.3;
      ...
      track.stop = () => { originalStop(); oscillator.stop(); void context.close(); };
...
test('Site navigation, mobile menu, articles and sitemap remain available', async ({ page, request, isMobile }) => {
  ...
  await page.locator('.tool-card').click(); await expect(page).toHaveURL(/sound-analyzer/);
