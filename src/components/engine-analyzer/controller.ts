...
import { analyzeEngine, captureEngineSnapshot, type EngineObservation, type EngineSnapshot } from '../../lib/domains/engine/analysis.ts';
...
export function mountEngineAnalyzer(root: HTMLElement): void {
  ...
  const handle = mountAnalyzer<EngineSnapshot>(root, {
    ...
    render: () => { renderEngine(root, observation, metadata); syncToolbar(); },
    reset: () => { tracker.reset(); spectrum = undefined; metadata = undefined; observation = undefined; lastMode = undefined; renderEngine(root); },
    capture: base => captureEngineSnapshot(base, observation!, metadata!),
    compare: (a, b) => renderEngineComparison(root, a, b),
