...
test('Live worklet engine analysis and A/B need no recording API', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('#analyzer-status')).toContainText('sample-clock capture');
...
test('Engine live works with blank references and falls back when AudioWorklet is missing', async ({ page }) => {
  ...
  await page.goto(route); await page.locator('#start-mic').click();
  await expect(page.locator('#analyzer-status')).toContainText('sampled fallback');
...
test('Worklet module failure and runtime processor failure degrade to usable sampled analysis', async ({ page }) => {
  ...
  await page.goto(route); await page.locator('#start-mic').click();
  await expect(page.locator('#analyzer-status')).toContainText('sampled fallback');
  await expect(page.locator('#save-a')).toBeEnabled(); await page.locator('#stop-analysis').click();
  ...
  await expect(page.locator('#analyzer-status')).toContainText('sampled fallback');
  await expect(page.locator('#analyzer-status')).toContainText('processor stopped');
  await expect(page.locator('#save-a')).toBeEnabled(); await page.locator('#stop-analysis').click();
...
test('Stop during worklet module loading prevents stale node/worker creation', async ({ page }) => {
  ...
  await page.goto(route); await page.locator('#start-mic').click();
  await expect.poll(() => page.evaluate(() => 'releaseWorklet' in window)).toBe(true); await page.locator('#stop-analysis').click();
...
test('A stalled live worker releases resources and falls back without unbounded messages', async ({ page }) => {
  ...
  await page.goto(route); await page.locator('#start-mic').click();
  await expect(page.locator('#analyzer-status')).toContainText('Continuous capture stalled.');
...
test('Engine permission cancellation and background cleanup do not revive stale input', async ({ page }) => {
  await syntheticMicrophone(page, true); await page.goto(route); await page.locator('#start-mic').click();
  await expect.poll(async () => (await audioState(page)).calls).toBe(1); await page.locator('#stop-analysis').click();
...
test('Engine file harmonics, incompatible contexts, invalid references and optional recording', async ({ page }) => {
  ...
  await page.getByText('Engine & harmonic references', { exact: true }).click(); await page.locator('#harmonic-hz').fill('100');
  ...
  await page.locator('#engine-rpm').fill('1800'); await page.locator('#start-mic').click();
...
test('Engine layout, references, routes and responsive controls', async ({ page, request }, testInfo) => {
  ...
  await page.getByText('Engine & harmonic references', { exact: true }).click(); await page.locator('#engine-cylinders').fill('4'); await page.locator('#engine-cycle').selectOption('4');
...
test('Engine unsupported audio reports errors and keeps references usable', async ({ page }) => {
  ...
  await page.goto(route); await page.locator('#start-mic').click(); await expect(page.locator('#analyzer-error')).toContainText('does not support');
  await page.locator('#engine-rpm').fill('1800'); await expect(page.locator('#rpm-reference-summary')).toContainText('30.00 Hz');
});
